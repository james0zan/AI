...
void print_help() {
  ERROR("[Usage] read-whitelist [.AccessesInfo File] [White List File]");
}

map<AI_ACCESS_ID, AccessInfo> accessInfoMap;
...
int main(int argc, char *argv[]) {
  ...
  FILE* whiteList = fopen(argv[2], "r");
  AI_ACCESS_ID ID;
  while (fscanf(whiteList, "%lu", &ID) != EOF) {
    printf("================\n");
    printAccess(ID);
  }
}